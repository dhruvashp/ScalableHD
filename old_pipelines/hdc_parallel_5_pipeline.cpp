...
int R = 2;                               // GUIDANCE: R <= ceil(F/f) 

// IMPORTANT NOTE for v5
// Eigen loves threads = cores
// We have Eigen launch 2*T threads
// As we launch 2*T pipelined threads
// Subsequently, T = 16 (INDUS)
...
int main() {
    ...
    // Dummy
    MatrixXfRowMajor Dummy_H = MatrixXfRowMajor::Random(N, D);
    ...
    for (int trial = 0; trial < trials; ++trial) {
        ...
        total_time_naive += chrono::duration<double, milli>(t2 - t1).count();
        ...
        Eigen::initParallel();
        Eigen::setNbThreads(2*T);       // 2x threads for fairness

        auto t_omp_start = chrono::high_resolution_clock::now();

        // Two independent multiplications as the best possible baseline to compete against
        H_omp.noalias() = X * B;
        S_omp.noalias() = Dummy_H * J;
        
        auto t_omp_end = chrono::high_resolution_clock::now();
        total_time_omp += chrono::duration<double, milli>(t_omp_end - t_omp_start).count();
    }

    double avg_time_naive = total_time_naive / trials;
    ...
    cout << "Average Naive Time over " << trials << " runs    : " << avg_time_naive << " ms\n";
    ...
    cout << "Speedup (Parallel): " << avg_time_naive / avg_time_parallel << "\n";
    cout << "Speedup (OpenMP)  : " << avg_time_naive / avg_time_omp << "\n";

    cout << "Parallel vs OpenMP: " << avg_time_omp / avg_time_parallel << "                (>1 Win, =1 Tie, <1 Lose) \n"; 
    cout << "Parallel Algorithm Optimality Ratio: " << (avg_time_naive / (avg_time_parallel * 2 * T)) << "\n";


    //print_matrix(Y_pred_naive);
