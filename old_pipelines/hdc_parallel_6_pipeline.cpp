...
int R = 32;                                                      // GUIDANCE: R <= ceil(F/f) 

// For Parallel (Task I -> T threads, Task II -> T threads)
int T = 32;                                                     // GUIDANCE: T <= max_threads_feasible/2              max_threads_feasible -> Total logical processor threads/CPUs to twice that amount
...
void task2_worker(int thread_id) {
    ...
    // pin_thread_to_logical_core(thread_id + T); 
